# Claudinho Local Agent Playbook

This playbook is a practical guide to run Claudinho with a local model (Ollama), work safely, and get strong day-to-day results.
...
If everything is healthy, Claudinho starts directly.
...
Or let Claudinho recommend the best local model for your goal:
...
```powershell
Get-Content .\.claudinho-profile.json
```
...
```powershell
cd C:\Users\Lucas Pedry\Documents\claudinho\claudinho
bun run dev:profile
```
...
- Keep `.claudinho-profile.json` local (already gitignored).
